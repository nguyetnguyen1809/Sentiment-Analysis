...
#---------CHUẨN BỊ DỮ LIỆU----------
file_path = r"D:\Sentiment Analysis\comments_data_ncds.csv"
...
path_nag = r"D:\Sentiment Analysis\nag.txt"
path_pos = r"D:\Sentiment Analysis\pos.txt"
path_not = r"D:\Sentiment Analysis\not.txt"
path_stopwords = r"D:\Sentiment Analysis\vietnamese-stopwords.txt"
...
plt.title('Phân phối độ dài câu dựa vào Đánh giá của khách hàng')
...
print("File đã được xuất ra file D:\Sentiment Analysis\comments_cleaned.csv")
output_path = r"D:\Sentiment Analysis\comments_cleaned.csv"
...
# Đọc file csv
file_path = r"D:\Sentiment Analysis\comments_cleaned.csv"
df = pd.read_csv(file_path)
...
# X_train, y_train: Tập huấn luyện /// X_test, y_test: Tập kiểm tra
...
def evaluate_model_kfold(model,param_grid, X, y, kfold):
    ...
    for train_ids, val_ids in  kfold.split(X, y):
        ...
        #Tính trung bình các chỉ số trên tất car các folds 
        # So sánh kết quả
    model_results = {
...
rects1 = ax.bar(x - width*1.5, accuracy, width, label='Accuracy', color='skyblue')
rects2 = ax.bar(x - width/2, precision, width, label='Precision', color='orange')
rects3 = ax.bar(x + width/2, recall, width, label='Recall', color='green')
...
new_text = ["gói hàng cũng tạm, hàng thì ổn", " Quyển sách này thật tệ", "Tôi thấy cái bình nước này rất tuyệt vời"]
